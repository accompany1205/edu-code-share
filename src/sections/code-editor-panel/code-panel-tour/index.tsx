...
import JoyRide, { type CallBackProps, type status, ACTIONS, EVENTS, STATUS, valueof } from "react-joyride";
  ...
  useEffect(() => {
    ...
    if (userCreated) {
      const isTourShouldBeStopped = (
        differenceInSeconds(new Date(), new Date(userCreated)) / MS_IN_SECOND > 1
      );
